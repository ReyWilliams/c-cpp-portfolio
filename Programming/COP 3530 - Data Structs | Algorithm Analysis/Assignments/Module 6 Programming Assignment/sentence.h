...
public:
	...
	void add_back(string& s);		//Add a word (node) to the back of the sentence(link list). 
	void add_front(string& s);		//Add a word (node) to the front the sentence(link list). - for future assignment
	...
	word* getFront(){return this->front;}	//returns the front pointer 
	word* getBack(){return this->back;}		//returns the back pointer 
private:
	word *front, *back;
};
