...
Due Date: July 27, 2020                      Total Points: 20
Assignment#: 9 (nursery_orders9.cpp)
...
void order_class::remove(const string key){
	...
	while(search(key) != -1){ //while there exists an index in the array that contains key

		index = search(key); //sets index to target index
...
int main()
{
    remove("nursery_result20.txt"); //deletes output file before each run
